...
int GraspitInterface::init(int argc, char** argv)
{
    ...
    plan_grasps_as = new actionlib::SimpleActionServer<graspit_interface_custom::PlanGraspsAction>(*nh, "planGrasps",
...
void GraspitInterface::PlanGraspsCB(const graspit_interface_custom::PlanGraspsGoalConstPtr &_goal)
{
    goal = *_goal;
...
// Handles PlanGrasps.action, reads input goal (class variable)
void GraspitInterface::runPlannerInMainThread()
{
    ...
    // Custom new parameter and fn to return customized number of top grasps
    int n_best_grasps = goal.n_best_grasps;
    if (n_best_grasps == 0)
      n_best_grasps = 20;
    mPlanner->setNumBestGrasps (n_best_grasps);
...
// Handles PlanGrasps.action, writes output result_ (class variable)
