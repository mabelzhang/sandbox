...
def main ():
  ...
  for w_i in range (len (obj_names)):
    ...
    for u_i in range (len (SUFFIXES)):
      ...
      grasps = GraspIO.read_grasps (os.path.basename (world_fname), SUFFIXES [u_i])
      cmeta = GraspIO.read_contact_meta (os.path.basename (world_fname), SUFFIXES [u_i])
      # For printing for debugging only
      energies = GraspIO.read_energies (os.path.basename (world_fname),
        ENERGY_ABBREV, SUFFIXES [u_i])
  ...
  for w_i in range (len (obj_names)):
    ...
    print ('Total %d grasps for %s' % (n_grasps [w_i], obj_names [w_i]))
